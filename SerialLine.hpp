...
public: // Output
    ...
    ///
    /// The returned number of bytes can be sent in one call
    /// of the `send` method. If you try to send more bytes, you will get
    /// `Partial` and exceeding bytes are ignored.
    ///
    ...
    /// @param[out] dataSent If this pointer is passed to the function,
    ///     the number of actually sent bytes is resturned. In this case
    ///     the status `Partial` is returned.
    ...
    ///
    /// This will reset the send buffer, dropping all data which
    ...
    ///
    /// This function tries to read the number of bytes passed in the
    /// given time frame. If not all data could be sent, the number of
    /// sent bytes are stored in variable `dataReceived`. To ignore the
    /// numnber of bytes sent, pass `nullptr` to this parameter.
    ///
    /// @param[out] data A pointer to a buffer to receive the data.
    /// @param[in] dataSize The maximum number of bytes to receive.
    /// @param[out] dataReceived If this pointer to a variable is passed to
    ///    the function, the actual number of bytes received is stored in the variable.
    ...
    /// similar, but also successfully stops after reading a byte equal
    /// to the `blockEndMark`. This end mark is read and added to the
    /// buffer `data`.
    ///
    /// @param[out] data A pointer to a buffer to receive the data.
    /// @param[in] dataSize The maximum number of bytes to receive.
    /// @param[in] blockEndMark The block end mark.
    /// @param[out] dataReceived The number of bytes received in the given timeframe.
    /// @return `Success` if the block end mark was read.
    ///    `Partial` the data block was filled with data, but no end mark was read.
    ///    `Timeout` if no end mark could be read and the data block was not completely filled.
    ///
